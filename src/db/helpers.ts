...
import seedLeadsRaw from './seed-leads-formatted.json' with { type: 'json' };

// Full benchmark lead intelligence records (all 193 verified leads)
export const DEFAULT_STORED_LEADS: any[] = Array.isArray(seedLeadsRaw) && seedLeadsRaw.length > 0
  ? seedLeadsRaw
  : [
      {
        ...
        vendorMentions: [],
        sources: [],
        actionableSalesPitch: "Acme Industrial is heavily locked into the SAP ecosystem.",
        isSaved: true
      }
    ];
