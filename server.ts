...
import { seedDefaultLeads, getAllLeadsFromDb, upsertLeadToDb, executeReportingQuery, getExistingLeadByCompanyName, DEFAULT_STORED_LEADS } from './src/db/helpers.ts';
...
// File storage path for persistent storage
...
app.get(['/api/health', '/api/v1/health'], async (req, res) => {
  ...
  try {
    ...
    dbStatus.error = err?.cause?.message || err?.message || String(err);
    (dbStatus as any).details = {
      code: err?.code || err?.cause?.code,
      sqlHost: process.env.SQL_HOST ? process.env.SQL_HOST.substring(0, 20) + '...' : 'undefined',
